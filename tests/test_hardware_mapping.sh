...
grep -Fq '"DX4600"' "$BIOS_CONTROL" || fail "BIOS adapter must retain the exact DX4600 guard"
...
grep -Fq 'dx4600.so' "$BIOS_CONTROL" || fail "BIOS adapter must require the DX4600 upstream plugin"
...
stock_curve=$(bios_fan_curve_stock_json)
grep -Fq '"profile":"stock-4600"' <<< "$stock_curve" || fail "DX4600 stock fan curve must come from the bundled hardware record"
grep -Fq '"system_pwm":"64,152,228,255"' <<< "$stock_curve" || fail "DX4600 stock fan curve must preserve the recovered mode-2 PWM points"

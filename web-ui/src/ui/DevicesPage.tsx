...
export function DevicesPage({
  ...
  const checkUpdate = useCallback(async (force = false) => {
    const data = await api<UpdateData>('/update/check', { query: force ? 'force=1' : '' }); setUpdate(data); return data;
  }, []);

  useEffect(() => { if (!hidden) { void checkUpdate(); void loadLogs().catch(() => {}); } }, [checkUpdate, hidden, loadLogs]);
  ...
  const updateAvailable = Boolean(update?.latest_version && update?.current_version && String(update.latest_version) !== String(update.current_version));
  ...
  return <section className="react-view" hidden={hidden} aria-label="设备与高级">
    <div className="device-layout">
      ...
      <article className="glass-surface update-surface"><div className="section-heading"><div><span className="section-kicker">APPLICATION UPDATE</span><h2>应用更新</h2></div><span className="soft-badge">{update?.reachable === false ? '检查失败' : updateAvailable ? '发现新版本' : update ? '已是最新' : '尚未检查'}</span></div><div className="update-content"><div className="update-copy"><div className="update-version"><span>当前版本</span><strong>v{String(update?.current_version || '1.9.0').replace(/^v/i, '')}</strong></div><h3>{updateAvailable ? `发现新版本 v${String(update?.latest_version)}` : '检查 GitHub Release 获取新版本'}</h3><p className="surface-help">发现新版本后，可下载安装包并在 fnOS 应用中心手动升级。</p><small>{update?.checked_at ? `检查于 ${new Date(Number(update.checked_at) * 1000).toLocaleString()}` : '不会自动安装，也不会在后台静默升级。'}</small></div><div className="update-actions"><button type="button" className="secondary-button" disabled={busy === 'update'} onClick={() => void withBusy('update', async () => { const data = await checkUpdate(true); onToast(data.reachable === false ? '暂时无法连接 GitHub Release' : '更新检查完成', data.reachable === false ? 'err' : 'ok'); })}><i className="bi bi-arrow-clockwise" aria-hidden="true" />检查更新</button>{update?.release_url ? <a className="secondary-button" href={String(update.release_url)} target="_blank" rel="noopener noreferrer"><i className="bi bi-card-text" aria-hidden="true" />更新说明</a> : null}{update?.download_url ? <a className="primary-button" href={String(update.download_url)} target="_blank" rel="noopener noreferrer"><i className="bi bi-download" aria-hidden="true" />下载升级包</a> : null}</div></div></article>

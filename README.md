...
| 电源灯 | 智能 / 全开模式下可单独配色 |
| BIOS 控制 | 直接集成 [UGREEN-NAS-Hardware](https://github.com/BearHero520/UGREEN-NAS-Hardware) 的 `ugreenctl`；DX4600 / DX4600+ / DX4600 Pro、DXP4800、DXP4800 Plus / Pro、DXP4800S、DXP480T Plus、DXP6800 Pro 优先经 `it87` hwmon 控制；全部精确映射机型均支持经安全识别的 PCI 有线网卡网络唤醒，以及“安全关机 + RTC 定时开机”计划 |
...
BIOS 控制直接集成 [BearHero520/UGREEN-NAS-Hardware](https://github.com/BearHero520/UGREEN-NAS-Hardware) 的 `ugreenctl` 与 DX4600 / DXP4800 / DXP4800 Plus / DXP4800S / DXP480T Plus / DXP6800 Pro 插件（MIT）。该项目负责精确 DMI 匹配、IT8613 身份及原厂驱动冲突检查、进程锁和寄存器映射；本应用负责 Web API 与页面展示。DX4600 系列、DXP4800、4800S、4800 Plus / Pro 与 6800 Pro 使用动态 `name=it8613` hwmon 节点。来自官方固件的 `hwmonitor`、`hwmonitor-480t` 与 `hwmonitor-amd` 都明确映射 `ethtool` 魔术包唤醒；网卡改名时仍只接受安全识别出的物理 PCI 有线接口，它不写 Super I/O 寄存器。DXP480T Plus 在 FNOS 的实机记录确认唯一 AQC113 有线网卡会改名为 `enp115s0`，该精确机型在原厂 `eth0`/`eth1` 名称都不存在时可安全回退至恰好一张物理 PCI 有线网卡。官方 `TimedShutdown` / `OnSched` 路径也已接入为“定时安全关机 + RTC 定时开机”：关机前必须成功重设下一次 RTC 唤醒，失败即取消关机。全部固件逆向写入会要求用户作一次页面显式风险确认；确认按精确机型和风险版本存入 NAS 应用配置，页面重开不再重复询问，但每次写入仍附加确认令牌，并继续由 `ugreenctl` 执行 `--force --apply`、芯片 ID、进程锁和回读保护；仍应先在实机验证。DXP480T Plus 的原厂驱动将 CPU PWM 映射到 `0x17/0x73`，并由 `set` 事务按 `0x16/0x6b`、`0x1e/0x7b` 成对写入系统风扇 1/2；页面不提供 sys1/sys2 单独写入。RPM 的 `0x0fff`/`0xffff` 无效值会显示为 0。映射来自该固件静态逆向，仍需实机验证；普通 DXP480T 不会因手动机型档案选择而绕过精确 DMI 保护。
...
自动温控同样是软件守护进程，不会将 `pwm*_enable=2` 标示为“原厂自动”。守护程序只调用已内置的 `ugreenctl`，每次写入继续经过精确 DMI、原厂控制器互斥、IT8613 锁、最低 PWM 与回读保护。DX4600 系列的 `stock-4600` 原厂兼容方案采用固件恢复的 CPU/HDD/NVMe 阈值和 mode 2 生效后的 64/152/228/255 PWM 点；DXP4800S 使用 64/128/204/255 PWM 点，安全实现都不会执行停转。CPU 温度缺失会保护性满速，用户可选择在缺少全部 HDD/NVMe 温度时同样满速；降速延迟用于避免频繁升降速。
...
| `/bios/status` | DX4600 / DX4600+ / DX4600 Pro、DXP4800、DXP4800 Plus / Pro、DXP4800S、DXP480T Plus、DXP6800 Pro 的 IT8613、风扇、来电启动、网络唤醒与定时开关机状态及写入能力标记 |
...
| `/bios/fan-curve` | GET：读取自动温控曲线状态；POST `action=start`：保存并启动与精确 DMI 匹配的 `stock-4600`、`stock-4800`、`stock-4800s`、`stock-4800plus`、`stock-480tplus`、`stock-6800pro` 或 `custom` 软件曲线；POST `action=stop`：停止并禁用开机恢复。受保护写入仍要求现有风险确认。 |
| `/bios/startup?policy=on\|off\|last` | POST：设置来电启动策略；DX4600 系列、DXP4800、4800S 与 6800 Pro 写入要求 `confirm=firmware-reversed` |
| `/bios/wol?policy=on\|off` | POST：精确 DMI `DXP4800`、`DXP4800S`、`DXP4800 Plus`、`DXP4800 Pro`、`DXP480T Plus`、`DXP6800 Pro`；设置经安全识别的物理 PCI 有线网卡魔术包网络唤醒并回读校验，要求 `confirm=firmware-reversed` |

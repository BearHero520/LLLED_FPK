...
| `/bios/fan-curve` | GET：读取自动温控曲线状态；POST `action=start`：保存并启动与精确 DMI 匹配的 `stock-4800s`、`stock-4800plus`、`stock-480tplus`、`stock-6800pro` 或 `custom` 软件曲线；POST `action=stop`：停止并禁用开机恢复。受保护写入仍要求现有风险确认。 |
